...
// Maps are built dynamically in the component using translations
    ...
    <View style={{ flex: 1, backgroundColor: Colors.BACKGROUND }}>
        ...
        <View>
          ...
          {mode === "owner" && activeDog && (
            <Text style={{ fontSize: 12, color: Colors.TEXT_MUTED, marginTop: 1 }}>
              {`${t.swipe_sub_gassidate} ${activeDog.name}`}
            </Text>
          )}
        </View>
      ...
      {mode === "adoption" && loading && (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
          ...
          <Text style={{ color: Colors.TEXT_MUTED, marginTop: 12 }}>{t.adoption_empty_title}</Text>
        </View>
      )}
      ...
      {mode === "owner" && myDogs !== null && myDogs.length === 0 && (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 32 }}>
          ...
          <TouchableOpacity
            ...
            <Text style={{ color: Colors.WHITE, fontWeight: "700", fontSize: Sizes.FONT_MD }}>{t.onb_th_btn}</Text>
          </TouchableOpacity>
        </View>
      )}
      {mode === "owner" && myDogs !== null && myDogs.length > 0 && loading && (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
          <ActivityIndicator color={Colors.SECONDARY} size="large" />
          <Text style={{ color: Colors.TEXT_MUTED, marginTop: 12 }}>{t.swipe_sub_gassidate}</Text>
        </View>
      )}
        ...
        <Animated.View style={{
          ...
          <Animated.View style={{
            ...
            <Text style={{ color: Colors.TEXT_MUTED, textAlign: "center", marginBottom: 24, lineHeight: 20 }}>
              {t.swipe_match_desc(matchPet?.shelter_name ?? t.adoption_chat_shelter_fallback)}
            </Text>
            <TouchableOpacity
              onPress={() => {
            ...
            >
              <Text style={{ color: Colors.WHITE, fontWeight: "700", fontSize: Sizes.FONT_MD }}>{t.adoption_feed_msg_btn}</Text>
            </TouchableOpacity>
...
function AdoptionPetView({
  ...
  const KINDER_MAP_LOCAL: Record<string, string> = {
    ja: t.adoption_children_yes,
    nein: t.adoption_children_no,
    ab_schulalter: t.adoption_children_school,
    ab_teenager: t.adoption_children_teen,
  };
  const AKTIV_MAP_LOCAL: Record<string, string> = {
    sportlich: t.adoption_activity_very,
    mittel: t.adoption_activity_medium,
    ruhig: t.adoption_activity_calm,
  };
  const ERFAHRUNG_MAP_LOCAL: Record<string, string> = {
    anfaenger: t.adoption_exp_beginner,
    fortgeschritten: t.adoption_exp_experienced,
    profi: t.adoption_exp_pro,
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} bounces style={{ flex: 1 }}>
        ...
        {pet.charakter_tags && pet.charakter_tags.length > 0 && (
          <View style={{ padding: 20, paddingTop: 18 }}>
            <Text style={{ fontSize: 13, fontWeight: "700", color: Colors.TEXT_MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>{t.pet_section_char}</Text>
        ...
        <View style={{ padding: 20, paddingTop: 18 }}>
          ...
          <PropRow icon="👦" label={t.pet_label_children}       value={KINDER_MAP_LOCAL[pet.kinderfreundlich ?? ""] ?? "–"}  ok={pet.kinderfreundlich !== "nein"} />
          ...
          <PropRow icon="🏃" label={t.pet_label_activity}       value={AKTIV_MAP_LOCAL[pet.aktivitaetslevel ?? ""] ?? "–"}  ok />
          {pet.erfahrung_benoetigt && (
            <PropRow icon="📚" label={t.pet_label_exp} value={ERFAHRUNG_MAP_LOCAL[pet.erfahrung_benoetigt] ?? "–"} ok={pet.erfahrung_benoetigt === "anfaenger"} />
          )}
          {pet.im_heim_seit && (
      ...
      <ScrollView showsVerticalScrollIndicator={false} bounces style={{ flex: 1 }}>
        ...
        <View style={{ height: W * 1.15, position: "relative" }}>
          ...
          <View style={{ position: "absolute", top: 14, right: 14, backgroundColor: Colors.SECONDARY, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 99 }}>
            <Text style={{ color: "#FFF", fontSize: 12, fontWeight: "700" }}>{t.swipe_badge_gassidate}</Text>
          </View>
        </View>
          ...
          <View style={{ flexDirection: "row", alignItems: "baseline", gap: 10, marginBottom: 4 }}>
            ...
            {pet.alter_jahre != null && (
              <Text style={{ fontSize: 17, color: Colors.TEXT_MUTED }}>
                {pet.alter_jahre === 0 ? t.comp_owner_puppy : pet.alter_jahre === 1 ? `1 ${t.comp_owner_year}` : `${pet.alter_jahre} ${t.comp_owner_years}`}
              </Text>
            )}
          </View>
        ...
        <View style={{ paddingHorizontal: 20, paddingBottom: 4 }}>
          <Text style={{ fontSize: 13, fontWeight: "700", color: Colors.TEXT_MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 14 }}>{t.onb_th_behavior}</Text>
          {pet.aktivitaetslevel && (
            <PropRow icon="🏃" label={t.filter_activity} value={AKTIV_MAP[pet.aktivitaetslevel] ?? pet.aktivitaetslevel} ok />
          )}
          <PropRow icon="👦" label={t.onb_th_child_friendly}   value={pet.kinderfreundlich ? t.onb_yes : t.onb_no}        ok={pet.kinderfreundlich} />
          <PropRow icon="🐾" label={t.onb_th_animal_friendly}  value={pet.vertraeglich_mit_tieren ? t.onb_yes : t.onb_no} ok={pet.vertraeglich_mit_tieren} />
        </View>
        ...
        <View style={{ flexDirection: "row", gap: 14, padding: 20, paddingTop: 24, paddingBottom: 44 }}>
          ...
          <TouchableOpacity onPress={() => flyOff("ja")} disabled={saving} style={{
            ...
            {saving ? <ActivityIndicator color={Colors.WHITE} />
              : <Text style={{ fontSize: 22, fontWeight: "700", color: Colors.WHITE }}>{`🐾  ${t.swipe_badge_gassidate}`}</Text>}
          </TouchableOpacity>
        </View>
      </ScrollView>
    </Animated.View>
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <View style={{ flex: 1, backgroundColor: Colors.BACKGROUND }}>
        ...
        <View style={{
          ...
          <Text style={{ fontSize: 22, fontWeight: "800", color: Colors.TEXT }}>{t.gassi_my_dogs}</Text>
              ...
              >
                ...
                <View style={{ flex: 1, marginLeft: 14 }}>
                  ...
                  <Text style={{ fontSize: 13, color: Colors.TEXT_MUTED, marginTop: 2 }}>
                    {[dog.rasse, dog.groesse_kategorie ? GROESSE_LABEL[dog.groesse_kategorie] : null].filter(Boolean).join(" · ") || t.gassi_mixed_breed}
                  </Text>
                  {hasFilter && (
                    <Text style={{ fontSize: 11, color: Colors.SECONDARY, marginTop: 2, fontWeight: "600" }}>{t.gassi_individual_filter_active}</Text>
                  )}
                </View>
